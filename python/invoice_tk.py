...
class InvoiceTable(ctk.CTkFrame):
    def __init__(self, master, mode="Patti", total_label=None, **kwargs):
        ...
        self.mode = mode
        # total_label is now required and always passed from InvoiceWindow
    ...
    def update_amounts(self):
        logging.info("update_amounts called")
        total = 0.0
        ...
        if self.total_label:
            logging.info(f"Setting total_label to Amount: ₹{total:.2f}")
    ...
    def add_row(self):
        ...
        for col in range(1, len(MODE_HEADERS[self.mode])-2):
            ...
            entry.bind("<FocusOut>", lambda e: self.update_amounts())
...
class InvoiceWindow(ctk.CTk):
    ...
    def _setup_header(self):
        ...
        self.date_label = ctk.CTkLabel(self.header_frame, font=("Arial", 14, "bold"), text_color=BACKGROUND_COLOR, anchor="e", justify="right")
    ...
    def _setup_mode_selection(self):
        ...
        for mode in ["Patti", "Kata", "Barthe"]:
            btn = ctk.CTkButton(
                ...
                text=mode,
                font=LABEL_FONT,
                fg_color=PRIMARY_COLOR if mode == self.mode_var.get() else ACCENT_COLOR,
                text_color=BACKGROUND_COLOR if mode == self.mode_var.get() else TEXT_COLOR,
    ...
    def _on_mode_button_click(self, mode):
        ...
        for m, btn in self.mode_buttons.items():
            if m == mode:
                btn.configure(fg_color=PRIMARY_COLOR, text_color=BACKGROUND_COLOR)
            else:
                btn.configure(fg_color=ACCENT_COLOR, text_color=TEXT_COLOR)
    ...
    def _setup_table(self):
        ...
        self.table_outer_frame = ctk.CTkFrame(self.main_frame, fg_color=BACKGROUND_COLOR)
        ...
        self.table_inner_frame = ctk.CTkFrame(self.table_canvas, fg_color=BACKGROUND_COLOR)
        ...
        # The table will be created after total_label is set up in _setup_bottom_section
        pass  # Table is created after bottom section setup
    ...
    def _setup_bottom_section(self):
        ...
        # Now create the table and pass the total_label
        self.table = InvoiceTable(self.table_inner_frame, mode=self.mode_var.get(), total_label=self.total_label)
        self.table.pack(fill="both", expand=True)

        # Button group (left)
        btn_frame = ctk.CTkFrame(self.bottom_frame, fg_color=BACKGROUND_COLOR)
