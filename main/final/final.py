...
class InvoiceApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        # Maximize the window
        self.state('zoomed')  # This will maximize the window on Windows
    ...
    def generate_print_content(self):
        ...
        header_fmt = ""
        if mode == "Patti":
            # Item(5) Pkt(3) Qty(3) Rate(5) Ham(3) Amt(7) = 26 + 6 spaces = 32
            header_fmt = "{:<5} {:>3} {:>3} {:>5} {:>3} {:>7}"
            headers = ["Item", "Pkt", "Qty", "Rate", "Ham", "Amt"]
        elif mode == "Kata":
            # Item(5) Net(3) Les(3) Rate(5) Ham(3) Amt(7) = 26 + 6 spaces = 32
            header_fmt = "{:<5} {:>3} {:>3} {:>5} {:>3} {:>7}"
            headers = ["Item", "Net", "Les", "Rate", "Ham", "Amt"]
        elif mode == "Barthe":
            # Item(4) Pkt(3) Wt(3) +/-(3) Rate(4) Ham(3) Amt(6) = 26 + 6 spaces = 32
        ...
        for row_data in self.rows:
            ...
            try:
                if mode == "Patti" and len(row_values) >= 6:
                    # Truncate item name to 5 chars and format numbers
                    lines.append(header_fmt.format(
                        row_values[0][:5],
                        row_values[1][:3],  # Limit Pkt to 3 chars
                        row_values[2][:3],  # Limit Qty to 3 chars
                        row_values[3][:5],  # Limit Rate to 5 chars
                        row_values[4][:3],  # Limit Hamali to 3 chars
                    ...
                    lines.append(header_fmt.format(
                        ...
                        row_values[3][:3],  # +/-
                        row_values[4][:4],  # Rate
                        row_values[5][:3],  # Hamali
                        row_values[6][:6]   # Amount
                    ))
        ...
        # --- Footer ---
        # Encode Kannada text with UTF-8
        kannada_text = "ನಾನು ಎಲ್ಲವೂ ಸರಿಯಾಗಿದೆ ಎಂದು ಪರಿಶೀಲಿಸಿದ್ದೇನೆ."
        lines.extend([
            "-" * max_width,
            f"Total Amount: {self.total_label.cget('text')}".center(max_width),
            "-" * max_width,
            "",
            kannada_text.encode('utf-8').decode('utf-8').center(max_width),
            "",
            "_" * max_width,
    ...
    def update_datetime(self):
        ...
        self.date_label.configure(text=datetime.now().strftime("%A, %d %B %Y %I:%M:%S %p"))
