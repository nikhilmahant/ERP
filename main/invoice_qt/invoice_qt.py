...
class InvoiceWindow(QMainWindow):
    ...
    def update_amounts(self):
        ...
        for row in range(self.table.rowCount()):
            try:
                ...
                amount = 0.0
                if mode == "Patti":
                    pkt = validate_float(self.table.cellWidget(row, 1).text())
                    qty = validate_float(self.table.cellWidget(row, 2).text())
                    rate = validate_float(self.table.cellWidget(row, 3).text())
                    hamali = validate_float(self.table.cellWidget(row, 4).text())
                    amount = (qty * rate) + (pkt * hamali)

                elif mode == "Kata":
                    net = validate_float(self.table.cellWidget(row, 1).text())
                    less = validate_float(self.table.cellWidget(row, 2).text())
                    final_wt = net * (1 - less / 100.0) if less < 100 else 0.0
                    rate = validate_float(self.table.cellWidget(row, 3).text())
                    hamali_rate = validate_float(self.table.cellWidget(row, 4).text())
                    packets = int(net / 60) if net > 0 else 0
                    amount = (final_wt * rate) + (packets * hamali_rate)

                elif mode == "Barthe":
                    pkt = validate_float(self.table.cellWidget(row, 1).text())
                    wt_per_pkt = validate_float(self.table.cellWidget(row, 2).text())
                    adj = validate_float(self.table.cellWidget(row, 3).text())
                    rate = validate_float(self.table.cellWidget(row, 4).text())
                    hamali_per_pkt = validate_float(self.table.cellWidget(row, 5).text())
                    total_qty = (pkt * wt_per_pkt) + adj
                    amount = (total_qty * rate) + (pkt * hamali_per_pkt)

                # Update amount label in the last column
                amount_label = self.table.cellWidget(row, self.table.columnCount() - 1)
                amount_label.setText(f"₹{amount:.2f}")
                total += amount

            except Exception as e:
                logging.error(f"Error calculating amount for row {row}: {e}")
        ...
        if mode == "Kata" and hasattr(self, 'kata_amount_entry'):
            try:
                ...
                logging.error(f"Error adding kata amount: {e}")
    ...
    def setup_header(self):
        header_frame = QFrame()
        header_frame.setStyleSheet("QFrame { background-color: #f8f9fa; border-radius: 10px; }")
        ...
        shree_label.setStyleSheet("color: #6c757d;")
    ...
    def setup_bottom_section(self):
        ...
        for btn in [add_row_btn, clear_btn, save_btn, print_btn]:
    ...
    def show_print_preview(self):
        try:
            ...
            QMessageBox.critical(self, "Preview Error",
                f"Error generating print preview: {str(e)}")
